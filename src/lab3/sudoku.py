...
def find_empty_positions(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.Tuple[int, int]]:
    ...
    # >>> find_empty_positions([['.', '2', '.'], ['4', '5', '6'], ['7', '8', '9']])
    # (0, 0)
...
def fill_one_variant(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    f = False
    for i in range(9):
        for j in range(9):
            if grid[i][j] == '.':
                v = find_possible_values(grid, (i, j))
                if len(v) == 1:
                    grid[i][j] = list(v)[0]
                    f = True
    if f:
        fill_one_variant(grid)
    return grid
...
if __name__ == "__main__":
    ...
    display(answer)
    if not answer:
        print(f"Puzzle can't be solved")
    else:
        display(answer)
